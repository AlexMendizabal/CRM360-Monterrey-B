...
class LiberacoesController extends AbstractController
{
      ...
      ")->fetchAllAssociative();
            ...
            if (count($res) > 0){
                ...
                for ($i=0; $i < count($res); $i++) {
                    ...
                    $res[$i]['materiaisSemEstoque'] = $connection->executeQuery("
                    EXEC PRC_LIBE_COME_CONS 
                    @ID_PARA = 2,
                ...
                ")->fetchAllAssociative();
                    ...
                    $res[$i]['detalhes'] = $connection->executeQuery("
                    EXEC PRC_LIBE_COME_CONS 
                        @ID_PARA = 3,
                        @ID_EMPR = {$res[$i]['codEmpresa']},
                        @NR_PEDI = {$res[$i]['nrPedido']}
  ...
  public function getDetalhesLiberacoes(Connection $connection, Request $request, $nrPedido, $codEmpresa):JsonResponse
  {
    try {
      ...
      ")->fetchAllAssociative();
        ...
        $res[0]['motivoTrava'] = $connection->executeQuery("
        ...
        $res[0]['totais'] = $connection->executeQuery("
                EXEC PRC_LIBE_COME_CONS 
                    @ID_PARA = 3,
                    @ID_EMPR = {$codEmpresa},
                    @NR_PEDI = {$nrPedido}
        ...
        $res[0]['materiaisSemEstoque'] = $connection->executeQuery("
                EXEC PRC_LIBE_COME_CONS 
                    @ID_PARA = 2,
                    @ID_EMPR = {$codEmpresa},
                    @NR_PEDI = {$nrPedido},
  ...
  public function getMotivosTrava(Connection $connection, Request $request, $nrPedido, $codEmpresa):JsonResponse
  {
    try {
      ...
      $res['motivos'] = $connection->executeQuery("
          EXEC PRC_PEDI_LIBE_CONS
            @ID_PARA = 1, 
            @ID_EMPR = {$codEmpresa},
            @NR_PEDI = {$nrPedido},
      ...
      $res['detalhes'] = $connection->executeQuery("
                EXEC PRC_LIBE_COME_CONS 
                    @ID_PARA = 3,
                    @ID_EMPR = {$codEmpresa},
                    @NR_PEDI = {$nrPedido}
