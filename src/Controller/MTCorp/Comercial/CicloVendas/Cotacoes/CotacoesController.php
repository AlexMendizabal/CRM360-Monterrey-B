...
use App\Module\Comercial\Service\CalculoMaterialService;
...
if (!isset($params['codVendedor'])) {
     ...
     * @return JsonResponse
     */
    public function getTipoCalculoMaterial(CalculoMaterialService $calculoService, Request $request, $codMaterial)
    {
        try {
            $codFormaPagamento = (int)$request->query->get("codFormaPagamento");
            $res = $calculoService->getTiposCalculo((int)$codMaterial, $codFormaPagamento);

            if (!empty($res) && !isset($res['message'])) {
                return FunctionsController::Retorno(true, null, $res, Response::HTTP_OK);
            } elseif (!empty($res) && isset($res['message'])) {
     ...
     * @return JsonResponse
     */
    public function postCalculoMaterial(CalculoMaterialService $calculoService, Request $request)
    {
        try {
            $params = json_decode($request->getContent(), true);
            if (!is_array($params)) {
            ...
            $res = $calculoService->calcularMaterial($params);
