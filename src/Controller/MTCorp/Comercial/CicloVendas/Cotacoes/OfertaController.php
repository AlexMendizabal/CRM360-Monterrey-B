...
class OfertaController extends AbstractController
{
    ...
    public function postOfertaRegistrar(Connection $connection, Request $request)
    {
        ...
        try {
            ...
            if ($data_items['monto_total'] === null)
                $data_error['monto_total'] = 'es necesario';
            if ($data_items['monto_total_bruto'] === null)
                $data_error['monto_total_bruto'] = 'es necesario';
    ...
    public function insertarItems($connection, $data, $id_almacen, $id_oferta)
    {
        ...
        // Obtener id_material desde codMate
        $cod_material = $data['codMate'] ?? ($data['articulo'] ?? null);
        if (!empty($cod_material)) {
            $id_material = $connection->fetchOne('SELECT ID_CODIGOMATERIAL FROM TB_MATE WHERE CODIGOMATERIAL = ?', [$cod_material]);
            if ($id_material) {
                $data_items['id_material'] = $id_material;
                $data_items['codigo_material'] = $cod_material;
            } else {
                $data_error['id_material'] = 'No se encontro el material';
            }
        } else {
            $data_error['id_material'] = 'es necesario codMate';
        }

        !empty($data['id_almacen_carrito']) ? $data_items['id_almacen_carrito'] = (int) $data['id_almacen_carrito'] : $data_items['id_almacen_carrito'] = (int) $id_almacen;
        ...
        $cantidad = isset($data['qtde']) ? round((float) $data['qtde'], 4) : (isset($data['cantidad']) ? round((float) $data['cantidad'], 4) : null);
        $data_items['cantidad'] = $cantidad;
        if ($data_items['cantidad'] === null) {
            $data_error['cantidad'] = 'es necesario qtde';
        }

        $descuentoSolicitado = isset($data['desconto']) ? round((float) $data['desconto'], 4) : (isset($data['descuento']) ? round((float) $data['descuento'], 4) : 0);
        $descuentoPermitido = isset($data['descuento_permitido_valor']) ? round((float) $data['descuento_permitido_valor'], 4) : 0;
        ...
        $precioBruto = isset($data['preciobruto']) ? round((float) $data['preciobruto'], 4) : (isset($data['precio_neto_unidad']) ? round((float) $data['precio_neto_unidad'], 4) : 0);
        $data_items['precio_neto_unidad'] = $precioBruto;

        $subtotalBruto = $cantidad * $precioBruto;
        $subtotalNeto = $subtotalBruto * (1 - ($descuentoSolicitado / 100));

        $data_items['subtotal_bruto'] = isset($data['valorTotalBruto']) ? round((float) $data['valorTotalBruto'], 4) : round($subtotalBruto, 4);
        $data_items['subtotal'] = isset($data['valorTotal']) ? round((float) $data['valorTotal'], 4) : round($subtotalNeto, 4);

        $data_items['codigo_almacen'] = $connection->fetchOne('SELECT CODIGO_ALMACEN FROM TB_DEPO_FISI_ESTO WHERE id = ?', [$data_items['id_almacen_carrito']]);
        $data_items['peso'] = isset($data['peso']) ? round((float) $data['peso'], 4) : (isset($data['peso_unidad']) ? round((float) $data['peso_unidad'], 4) : 0);
        $data_items['subtotal_bruto_bs'] = isset($data['totalbs']) ? round((float) $data['totalbs'], 4) : null;
        $data_items['peso_total'] = $cantidad * $data_items['peso'];
        $data_items['precio'] = $precioBruto;
        try {
    ...
    public function getStockMaterial(Connection $connection, Request $request): JsonResponse
    {
        ...
        $FunctionsController = new FunctionsController();
        $helper = new Helper(); // Helper para realizar las consultas a SAP
        $helperSap = new HelperSap(); // Helper para actualizar los datos desde SAP
            ...
            if ($dataSap['CodigoRespuesta'] == 200) {
                foreach ($dataSap['Mensaje'] as $datos) {
                    ...
                    if (empty($actualizarPrecio)) {
                        $helperSap->insertarPrecio($connection, $id_item, $datos['Lugar'], $datos['Precio'], $datos['Peso'], $codMate, $id_unidad);
                    }
                }
            } else {
        ...
        // Consulta SQL a la base de datos para obtener el stock actualizado
        ...
        // Retornamos el stock actualizado
                ...
                if ($oferta_realizada['responseCode'] == 200 && $carrito != null) {
                    ...
                    if ($detaEliminado == true) {
                        ...
                        if ($resp) {
                            ...
                            $sapmsj = $helper->autorizacion_estado_sap($connection, $id_oferta);
