    ...
    public function actualizarItem(Connection $connection, Request $request)
    {
        try {
            ...
            if (isset($item_code) &&  isset($item_name) && isset($clase) &&  isset($unidad) && isset($peso) && isset($estado)) {
                ...
                $_rowsBuscar = $_resultBuscar->fetchAllAssociative();

                if (count($_rowsBuscar) > 0) {
                    ...
                    $affectedRows = $actualizar_material->executeStatement();
                    if ($affectedRows > 0) {
                        $message = array(
                    ...
                    $affectedRowsInsert = $guardar_material->executeStatement();
                }

                if (isset($affectedRowsInsert) && $affectedRowsInsert > 0) {
                    $message = array(
                        'CodigoRespuesta' => true,
