...
class LotesDuplicadosController
{
     ...
     * @return JsonResponse
     */
    public function show(Connection $connection, Request $request):JsonResponse
    {
        try {

            $id                     = $request->query->get("ID_LOGI_ENMA_NFMA_LTDP");	
            $notaFiscalMaterialId   = $request->query->get("ID_LOGI_ENMA_NOFI_MATE");
            $status                 = $request->query->get("IN_STAT");
            $inPagi                 = $request->query->get("IN_PAGI");
            $pagina                 = $request->query->get("PAGI");   
            $inObservacao           = $request->query->get("IN_DS_OBSE");   
            ...
            $query = <<<SQL
                EXECUTE PRC_LOGI_ENMA_NFMA_LTDP
                     @PARAMETRO                 = 2
                    ,@ID_LOGI_ENMA_NFMA_LTDP    = '{$id}'
                    ,@ID_LOGI_ENMA_NOFI_MATE    = '{$notaFiscalMaterialId}'
                    ,@IN_STAT                   = '{$status}'
                    ,@PAGI                      = '{$pagina}'
                    ,@IN_DS_OBSE                = '{$inObservacao}'
    ...
    public function store(Request $request, Connection $connection)
    {
        try {
            ...
            $id                     = isset($data->ID_LOGI_ENMA_NFMA_LTDP)  ? $data->ID_LOGI_ENMA_NFMA_LTDP     : '';
            $notaFiscalMaterialId   = isset($data->ID_LOGI_ENMA_NOFI_MATE)  ? $data->ID_LOGI_ENMA_NOFI_MATE     : '';
