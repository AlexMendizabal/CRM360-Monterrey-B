...
class SapController extends AbstractController
{
    ...
    public function sapInsertCliente(Connection $connection, Request $request)
    {
        ...
        $helper->verificarCliente($connection, $codigo_cliente);
        */
        $insertCliente = $helper->insertClient($connection, $data);
        if ($insertCliente['estado']) {
            ...
            foreach ($data['ubicacion'] as $key => $value) {
                ...
                if ($ubClie[$key]['estado'] == false) {
                    $msj = $ubClie[$key]['detalle'];
                }
            }

            foreach ($data['contactos'] as $key => $value) {
                $contacto[] =  $helper->insertContacto($connection, $value, $id_cliente);
                if ($contacto[$key]['estado'] == false) {
                    $msj = $contacto[$key]['detalle'];
                }
            }
          ...
          /*  if (empty($codigo_cliente) and $codigo_cliente == '' and $codigo_cliente == null) {
                $ruta = '/crearCliente';
                $respuesta = $helper->insertarServicio($ruta, $data);
            } */
